...
有了分支管理和 PR 之後，多人協作不會再互相蓋掉程式碼。但每次有人發 PR，還是得靠人記得去跑測試、檢查程式碼風格、確認編譯有沒有過。有時候忘了跑就按了合併，結果壞掉的程式碼又進到 `main` 裡面。協作流程有了，但這些檢查不應該靠人記得去做 — 這就是 CI 要解決的事。

跑完第一個 Hello World workflow 之後，這章把 CI 用到真實的 Go 專案上。
...
**CI（Continuous Integration）** 的核心精神是：每次有改動就自動把它整合進共用分支、跑一輪驗證流程，確認這次改動沒有打壞既有功能。實際上要跑哪些檢查由團隊決定，常見的有 lint、單元測試、編譯、type-check、container image 安全掃描、licence 檢查等等，依專案需求增減。

這些檢查全部自動執行，開發者不需要記得「提交前要先跑哪些步驟」。如果任何一步失敗，系統會直接擋住合併，開發者收到通知後去修就好。

本章的 sample-app 是個典型的 Go 服務，CI pipeline 會涵蓋下面三項，這也是大多數 Go 專案最常見的組合：
