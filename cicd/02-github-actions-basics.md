...
> 完整檔案也可以在 `cicd/examples/.github/workflows/hello.yml` 找到，直接複製過去即可。
...
`uses` 的格式是 `{owner}/{repo}@{version}`，例如 `actions/checkout@v4` 就是 GitHub 官方提供的 checkout Action 第 4 版通常會使用版本號（用 `@v4` 而非 `@main`）以避免未預期的變更。常用的 Actions 可以在 [GitHub Marketplace](https://github.com/marketplace?type=actions) 找到。
...
這五個元素（Workflow / Event / Job / Step / Runner）構成了 workflow 的結構骨架，表達式與 context 則是貫穿其中的 runtime 變數機制。用剛剛的 `hello.yml` 來看這些結構元素的階層關係：
...
在實際開發中，CI pipeline 失敗是常態，學會看失敗訊息、知道從哪裡 debug 是 SRE 的基本功。我們接下來要拿一個會失敗的 pipeline 來跑跑看。請在 `.github/workflows/repo-info.yml` 中建立這個檔案。這個 workflow 會去呼叫 GitHub API 拿這個 repository 的資訊，然後用 `jq` 解析 JSON：
...
push 上去之後到 Actions 分頁看，這次的 workflow run 旁邊會變成紅色叉叉。點進去你會看到前兩個 step（`Fetch repository info` 和 `Show repository name`）都還是綠色勾勾，但 `Show star count` 失敗了，錯誤訊息是 `jq: error: Could not find field 'stars'`，後面跟著 `Error: Process completed with exit code 1.`。更值得注意的是失敗那一步後面的 `Show description` 完全沒被執行——這是 GitHub Actions 的預設行為，**只要任何一個 step 失敗（exit code 不是 0），後面的 step 就不會跑**，整個 job 也會被標記為失敗。

可以試著想想看為什麼會失敗：GitHub API 回傳的 JSON 裡，star 數的欄位其實叫 `stargazers_count`，不是 `stars`。把 `.stars` 改成 `.stargazers_count` 再 push 一次，workflow 就會回到綠色狀態。
